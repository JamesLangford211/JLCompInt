...
public class ExpressionConverter {
	...
	public ExpressionConverter(String Expression, EXPRESSIONTYPE Type)
	{
		switch (Type)
		{
			case Prefix:
	...
	private ExpressionNode ConstructPrefixExpressionTree(String PrefixExpression)
	{
		...
		{
			if (ExpressionConverter.OPERATIONS.indexOf(Term) == -1)
			{
				try
				{
			...
			{
				Nodes.add(new ExpressionNode(ExpressionConverter.GetOperation(ExpressionConverter.OPERATIONS.indexOf(Term))));
			}
		}
	...
	private ExpressionNode ConstructInfixExpressionTree(String InfixExpression)
	{
		...
		{
			if (ExpressionConverter.OPERATIONS.indexOf(InfixExpression.charAt(i)) != -1)
			{
			...
			for (int i = 0; i < Nodes.size(); ++i)
			{
				switch (Nodes.get(i).toString().charAt(0))
				{
					...
					case '^':
						...
						while ((OperationStack.size() > 0) && (OperationStack.get(0).toString().equals("(") == false) && (ExpressionConverter.Precedence(Nodes.get(i).toString()) <= ExpressionConverter.Precedence(OperationStack.get(0).toString())))
						{
							ValueStack.add(0, OperationStack.remove(0));
							ValueStack.get(0).SetChildren(ValueStack.remove(2), ValueStack.remove(1));
						}
	...
	private class ExpressionNode
	{
		...
		{
			return String.valueOf(ExpressionConverter.OPERATIONS.charAt(this.Operation.ordinal()));
		}
		public String GetPrefixExpression()
		{
